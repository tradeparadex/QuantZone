...
class ParadexPerpConnector(ConnectorBase):
    ...
    async def initialize(self):
        ...
        _paradex_perpetual_chain = (
            PROD
            if _para_env == "mainnet"
            else (TESTNET if _para_env == "testnet" else _para_env)
        )
    ...
    def cancel_order(self, client_order_id, by="exchange_order_id"):
        ...
        if by == "client_order_id":
            ...
            if exchange_order_id is None:
                self.logger.warning(
                    f"{client_order_id=}, {order_to_cancel} does not have exchange_order_id assigned, will revert back to canceling by 'client_order_id'"
                )
                cancel_method = cancel_via_client_order_id
                oid = client_order_id
            else:
        ...
        try:
            ...
            if ve.data["message"] == "rate limit exceeded":
                ...
                self.logger.error(
                    f"Unexpected validation error for order {client_order_id}: {ve.data}"
                )
    ...
    def sync_open_orders(self, mkt):
        ...
        existing_orders = []
        try:
            p_orders = self.rate_limited_request(
                self.paradex.api_client.fetch_orders, {"market": mkt}
            )
        ...
        for ao in set(self.active_orders.keys()) - set(already_processed):
            try:
                po = self.rate_limited_request(
                    self.paradex.api_client.fetch_order_by_client_id, ao
                )
                self.process_order_update(po)
            except ValidationError as ve:
                if ve.data["message"] == "rate limit exceeded":
                    ...
                    self.logger.error(
                        f"Unexpected validation error for order {ao}: {ve.data}"
                    )
            except Exception as e:
                if e.args[0].error == "CLIENT_ORDER_ID_NOT_FOUND":
    ...
    def cancel_all_orders(self, mkt):
        ...
        self.rate_limited_request(
            self.paradex.api_client.cancel_all_orders, {"market": mkt}
        )
    ...
    def insert_order(self, order: Order):
        ...
        _order_side = (
            ParadexOrderSide.Buy if order.side == Side.BUY else ParadexOrderSide.Sell
        )
    ...
    async def _on_bbo(self, channel, msg):
        ...
        _latest_bbo = {
            "ask": Level(
                px=_data["ask"], qty=_data["ask_size"], offset=_data["seq_no"]
            ),
            "bid": Level(
                px=_data["bid"], qty=_data["bid_size"], offset=_data["seq_no"]
            ),
        }
    ...
    async def _on_fills(self, channel, msg):
        ...
        self.internal_positions[_market]["size"] += D(_data["size"]) * (
            -1 if _data["side"] == "SELL" else 1
        )

        self.logger.info(
            f"fills current size: {self.internal_positions[_market]['size']}"
        )
    ...
    def process_order_update(self, _data):
        ...
        if _data["status"] == "NEW":
            ...
            if _market in self._trade_callbacks:
                self._trade_callbacks[_market](
                    self.active_orders[_data["client_id"]], UpdateType.ORDER_UPDATE
                )
    ...
    async def _on_positions(self, channel, msg):
        ...
        if (
            _data["market"] in self.orderbooks
            and _data["market"] in self.internal_positions
        ):
            if (
                self.internal_positions[_data["market"]]["fill_id"]
                == _data["last_fill_id"]
            ):
    ...
    def get_positions(self) -> Dict[str, Position]:
        return {
            x: Position(
                ...
                D(self.positions[x]["cost_usd"])
                + D(self.positions[x]["unrealized_pnl"]),
            )
    ...
    async def subscribe_to_account_channels(self, callback: Callable = None):
        ...
        await self.paradex.ws_client.subscribe(
            ParadexWebsocketChannel.POSITIONS, callback=self._on_positions
        )

        await self.paradex.ws_client.subscribe(
            ParadexWebsocketChannel.TRADEBUSTS, callback=self._on_trade_bust
        )
