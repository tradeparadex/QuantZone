...
def load_trades(data_config: DataConfig = DataConfig()):
    ...
    for market in markets["symbol"]:
        ...
        try:
            ...
            logger.info(
                f"Latest trade date: {start_at} / {datetime.fromtimestamp(start_at/1000) if start_at else 'None'}"
            )
...
def load_markets_summary_single(
    market: str, start_at: int | None = None, end_at: int | None = None, data_config: DataConfig = DataConfig()
):
...
def load_markets_summary(data_config: DataConfig = DataConfig()):
    ...
    for market in markets["symbol"]:
        ...
        try:
            ...
            start_at = None if latest["max_date"].iloc[0] is None else latest["max_date"].iloc[0]
            logger.info(
                f"Latest markets summary date: {start_at} / {datetime.fromtimestamp(start_at/1000) if start_at else 'None'}"
            )
        except duckdb.IOException:
