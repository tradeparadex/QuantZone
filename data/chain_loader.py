...
class StarkwareETHProxyCheck(ProxyCheck):
    async def implementation_address(self, address: Address, client: Client) -> int | None:
        return await self.get_implementation(
            address=address, client=client, get_class_func=client.get_class_hash_at, regex_err_msg=r"(is not deployed)"
        )

    async def implementation_hash(self, address: Address, client: Client) -> int | None:
        return await self.get_implementation(
            address=address, client=client, get_class_func=client.get_class_by_hash, regex_err_msg=r"(is not declared)"
        )
    ...
    def _get_implementation_call(address: Address) -> Call:
        return Call(to_addr=address, selector=get_selector_from_name("implementation"), calldata=[])


def get_proxy_config():
    return ProxyConfig(
        max_steps=5, proxy_checks=[StarkwareETHProxyCheck(), ArgentProxyCheck(), OpenZeppelinProxyCheck()]
    )
...
class DataConfig:
    ...
    def full_node_client(self) -> FullNodeClient:
        if self._full_node_client is None:
            ...
            self._full_node_client = FullNodeClient(node_url=config.starknet_fullnode_rpc_url)
    ...
    def load_contracts(self) -> None:
        ...
        paraclear_contract = asyncio.run(
            Contract.from_address(address=paraclear_address, provider=self.full_node_client(), proxy_config=True)
        )
        self.paraclear_contract = paraclear_contract

        usdc_address = self.paradex_system_config.bridged_tokens[0].l2_token_address
        usdc_contract = asyncio.run(
            Contract.from_address(
                address=usdc_address, provider=self.full_node_client(), proxy_config=get_proxy_config()
            )
        )
        self.usdc_contract = usdc_contract

        oracle_address = self.paradex_system_config.oracle_address
        oracle_contract = asyncio.run(
            Contract.from_address(address=oracle_address, provider=self.full_node_client(), proxy_config=True)
        )
        ...
        self.contract_names[paraclear_contract.address] = decode_shortstring(
            tuple(asyncio.run(paraclear_contract.functions["getName"].call()))[0]
        )
        self.contract_names[usdc_contract.address] = decode_shortstring(
            tuple(asyncio.run(usdc_contract.functions["name"].call()))[0]
        )
        self.contract_names[oracle_contract.address] = decode_shortstring(
            tuple(asyncio.run(oracle_contract.functions["getName"].call()))[0]
        )
...
def process_block(data_config: DataConfig = DataConfig(), block_number: int = 1):
    ...
    block_events = asyncio.run(
        data_config.full_node_client().get_events(
