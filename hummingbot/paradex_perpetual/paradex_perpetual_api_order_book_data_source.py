...
class ParadexPerpetualAPIOrderBookDataSource(PerpetualAPIOrderBookDataSource):
    ...
    async def _request_order_book_snapshot(self, trading_pair: str) -> Dict[str, Any]:
        ...
        data = await self._connector._api_get(
            path_url=f"{CONSTANTS.SNAPSHOT_REST_URL}/{ex_trading_pair}", data={}
        )
        return data
        ...
        try:
            for trading_pair in self._trading_pairs:
                ...
                trades_payload = {
                    ...
                    "method": "subscribe",
                    "params": {
                        "channel": CONSTANTS.TRADES_ENDPOINT_NAME.format(market=symbol)
                    },
                }
                ...
                order_book_payload = {
                    ...
                    "method": "subscribe",
                    "params": {
                        "channel": CONSTANTS.DEPTH_ENDPOINT_NAME.format(market=symbol)
                    },
                }
    ...
    def _channel_originating_message(self, event_message: Dict[str, Any]) -> str:
        ...
        if "result" not in event_message:
            ...
            if stream_name.startswith("order_book.") and stream_name.endswith(
                ".deltas"
            ):
    ...
    async def _parse_order_book_snapshot_message(
        ...
        if _data["update_type"] not in ["s"]:
            self.logger().warning(
                f"Unknown order book update type {_data['update_type']}"
            )
            return
