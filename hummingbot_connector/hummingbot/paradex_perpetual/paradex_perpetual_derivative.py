...
POS_SIDE_MAP = {"LONG": PositionSide.LONG, "SHORT": PositionSide.SHORT}
...
class ParadexPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    async def _update_trading_rules(self):
        exchange_info = await self._api_get(path_url=self.trading_rules_request_path, data={})
    ...
    async def _initialize_trading_pair_symbol_map(self):
        try:
            ...
            exchange_info = await self._api_get(path_url=self.trading_pairs_request_path, data={})
    ...
    async def _status_polling_loop_fetch_updates(self):
        await safe_gather(
            self._update_trade_history(), self._update_order_status(), self._update_balances(), self._update_positions()
        )
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        self.logger().debug(f"The order {order_id} does not exist on Paradex Perpetuals. " f"No cancelation needed.")
        await self._order_tracker.process_order_not_found(order_id)
        raise OSError(f'{cancel_result["response"]["data"]["statuses"][0]["error"]}')
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        side_map = {TradeType.BUY: ParadexOrderSide.Buy, TradeType.SELL: ParadexOrderSide.Sell}
        ...
        if order_result.get("error") is not None:
            raise OSError(f"Error submitting order {order_id}: {order_result['message']}")
            ...
            try:
                for symbol in symbols:
                    _response = await self._api_get(
                        path_url=CONSTANTS.ACCOUNT_TRADE_LIST_URL, is_auth_required=True, params={"market": symbol}
                    )
                    all_fills_response.extend(_response["results"])
            except asyncio.CancelledError:
                raise
            except Exception as request_error:
                self.logger().warning(f"Failed to fetch trade updates. Error: {request_error}", exc_info=request_error)
    ...
    async def _request_order_status(self, tracked_order: InFlightOrder) -> OrderUpdate:
        ...
        return self._get_order_update_object(
            order_update, trading_pair=tracked_order.trading_pair, client_order_id=client_order_id
        )
    ...
    def _process_order_message(self, order_msg: dict[str, Any]):
        ...
        order_update = self._get_order_update_object(
            order_msg, trading_pair=tracked_order.trading_pair, client_order_id=client_order_id
        )
        self._order_tracker.process_order_update(order_update=order_update)

    def _get_order_update_object(
        self, order_msg: dict[str, Any], trading_pair: str, client_order_id: str
    ) -> OrderUpdate:
        ...
        if _state == OrderState.CANCELED:
            if order_msg["cancel_reason"] is not None and order_msg["cancel_reason"] not in [
                "USER_CANCELED",
                "EXPIRED",
                "",
            ]:
    ...
    async def _format_trading_rules(self, exchange_info_dict: list) -> list[TradingRule]:
        ...
        self.coin_to_asset = {asset["symbol"]: asset for asset in exchange_info_dict["results"]}
    ...
    async def _get_last_traded_price(self, trading_pair: str) -> float:
        ...
        response = await self._api_get(path_url=CONSTANTS.TICKER_PRICE_CHANGE_URL, data={"market": exchange_symbol})
        price = 0
    ...
    async def _update_balances(self):
        ...
        """
        balance_info = await self._api_get(path_url=CONSTANTS.BALANCES_INFO_URL, is_auth_required=True)
    ...
    async def _update_positions(self):
        positions = await self._api_get(path_url=CONSTANTS.POSITION_INFORMATION_URL, data={}, is_auth_required=True)
