...
class ParadexPerpetualUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        extra_headers = {"Authorization": f"Bearer {self._auth.paradex_account.jwt_token}"}
    ...
    async def _subscribe_channels(self, websocket_assistant: WSAssistant):
        ...
        try:
            ...
            subscribe_fills_request: WSJSONRequest = WSJSONRequest(payload=fills_payload, is_auth_required=True)
    ...
    async def _process_event_message(self, event_message: dict[str, Any], queue: asyncio.Queue):
        ...
        if event_message.get("error") is not None:
            ...
            raise OSError({"label": "WSS_ERROR", "message": f"Error received via websocket - {err_msg}."})
        else:
