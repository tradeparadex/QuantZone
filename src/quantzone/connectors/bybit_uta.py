...
class BybitUTAConnector(ConnectorBase):
    ...
    _http_client: HTTP | None
    _socket_manager: WebSocket | None
    _private_socket_manager: WebSocket | None
    ...
    async def initialize(self):
        ...
        self._http_client = HTTP(testnet=testnet, demo=testnet, api_key=self.key, api_secret=self.secret)
        ...
        self._ws_client = WebSocket(
            testnet=testnet, channel_type=self.channel_type, trace_logging=False, retries=8888, restart_on_error=True
        )
    ...
    def insert_order(self, order: Order):
        ...
        try:
            _resp = self._http_client.place_order(
    ...
    def _listen_to_data(self, message):
        ...
        _up_dict = {
            "bids": [{"price": x[0], "size": x[1], "offset": data["seq"]} for x in data["b"]],
            "asks": [{"price": x[0], "size": x[1], "offset": data["seq"]} for x in data["a"]],
        }
    ...
    def get_account_info(self) -> AccountInfo:
        ...
        return AccountInfo(
            free_collateral=D(usdc_wallet["availableToWithdraw"]), account_value=D(usdc_wallet["equity"])
        )
