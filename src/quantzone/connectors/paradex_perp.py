...
class ParadexPerpConnector(ConnectorBase):
    ...
    def get_account_str(self):
        ...
        assert self.paradex is not None
        assert self.paradex.account is not None
        return str(self.paradex.account.l2_address)

    async def initialize(self):
        """Initialize the Paradex client with environment-specific settings."""
        _para_env = os.environ["PARADEX_ENVIRONMENT"]
        match _para_env:
            case "mainnet":
                _paradex_perpetual_chain = PROD
            case "testnet":
                _paradex_perpetual_chain = TESTNET
            case _:
                raise ValueError(f"Invalid environment: {_para_env}")
    ...
    def cancel_order(self, client_order_id, by="exchange_order_id"):
        ...
        if by == "client_order_id":
            ...
            if by != "exchange_order_id":
                self.logger.warning(
                    f"{by=} is none of 'exchange_order_id' nor 'client_order_id', defaulting to 'exchange_order_id'"
                )
            ...
            if exchange_order_id is None:
                self.logger.warning(
                    f"{client_order_id=}, {order_to_cancel} does not have exchange_order_id assigned, "
                    "will revert back to canceling by 'client_order_id'"
                )
                cancel_method = cancel_via_client_order_id
                oid = client_order_id
            else:
        ...
        try:
            _resp = self.rate_limited_request(cancel_method, oid)
            self.active_orders[client_order_id].status = "CANCELLING"
        except ValidationError as ve:
            if isinstance(ve.data, dict) and ve.data["message"] == "rate limit exceeded":
    ...
    def sync_open_orders(self, mkt):
        ...
        for ao in set(self.active_orders.keys()) - set(already_processed):
            try:
                ...
                if ve.data is not None and ve.data["message"] == "rate limit exceeded":
                    self.logger.warning("rate limit exceeded")
                else:
    ...
    async def _on_orderbook(self, channel, msg):
        ...
        if _symbol not in self.orderbooks:
            self.orderbooks[_symbol] = Depth(str(_ticker))
        ...
        if _data["market"] in self.orderbooks and _data["market"] in self.internal_positions:
            if self.internal_positions[_data["market"]]["fill_id"] == _data["last_fill_id"]:
                if self.internal_positions[_data["market"]]["size"] != D(_data["size"]):
                    force_sync = True
                    self.logger.info(
                        f"pos vs fills size: {self.internal_positions[_data['market']]['size']} vs {_data['size']}"
                    )
    ...
    def get_avg_entry_price(self, mkt: str) -> D | None:
    ...
    def get_positions(self) -> dict[str, Position]:
        return {
            x: Position(
                x,
                D(self.positions[x]["size"]),
                D(self.positions[x]["cost_usd"]) + D(self.positions[x]["unrealized_pnl"]),
            )
    ...
    def get_account_info(self) -> AccountInfo:
        return AccountInfo(
            free_collateral=D(self.account_info["free_collateral"]), account_value=D(self.account_info["account_value"])
        )
    ...
    async def subscribe_to_data_channels(self, market, callback):
        ...
        await self.paradex.ws_client.subscribe(
            ParadexWebsocketChannel.BBO, callback=self._on_bbo, params={"market": market}
        )

        await self.paradex.ws_client.subscribe(
            ParadexWebsocketChannel.FILLS, callback=self._on_fills, params={"market": market}
        )

        await self.paradex.ws_client.subscribe(
            ParadexWebsocketChannel.ORDER_BOOK_DELTAS, callback=self._on_orderbook, params={"market": market}
        )

        await self.paradex.ws_client.subscribe(
            ParadexWebsocketChannel.FUNDING_DATA, callback=self._on_funding_data, params={"market": market}
        )
    ...
    async def subscribe_to_trade_channels(self, market, callback):
        ...
        await self.paradex.ws_client.subscribe(
            ParadexWebsocketChannel.ORDERS, callback=self._on_order_update, params={"market": market}
        )

        await self.paradex.ws_client.subscribe(
            ParadexWebsocketChannel.TRADES, callback=self._on_trades, params={"market": market}
        )

        self.sync_open_orders(market)

    async def subscribe_to_account_channels(self, callback: Callable | None = None):
    ...
    async def start(self, account_callback: Callable | None = None):
