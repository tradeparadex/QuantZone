...
from .data_methods import PriceType
from .portfolio_margin_stats import PortfolioMarginState
...
class RiskManager:
    ...
    parent: "PerpMarketMaker"
    cached_portfolio_margin_state: PortfolioMarginState | None
    last_portfolio_margin_state_ts: int
    ...
    async def portfolio_margin_state(self) -> PortfolioMarginState:
        if (
            self.last_portfolio_margin_state_ts == 0
            or self.parent.now_ms() - self.last_portfolio_margin_state_ts > 20 * 1000
        ):
        ...
        assert self.cached_portfolio_margin_state is not None
    ...
    def any_data_stale(self) -> bool:
        ...
        if market_latency > self.parent.max_market_latency_ms:
            ...
            for ob in self.parent.market_connector.orderbooks.values():
                self.logger.info(
                    f"Market book: {ob.received_ts-ob.exchange_ts}, {dt.datetime.fromtimestamp(ob.received_ts/1e9)} "
                    f"{dt.datetime.fromtimestamp(ob.exchange_ts/1e9)}"
                )
            return True
    ...
    async def _get_portfolio_open_size(self) -> PortfolioMarginState:
        ...
        unrealized_pnl: D = D(
            sum(D(p["unrealized_funding_pnl"]) + D(p["unrealized_pnl"]) for p in account_positions.values())
        )
        position_long: D = D.max(D("0"), sum(positions))
        position_short: D = abs(D.min(D("0"), sum(positions)))
        open_notional: D = D(sum(abs(p) for p in positions))
